...
Variant: fixed 20 counties per state (Kansas and Nebraska).

Pipeline:
1) NASA daily -> weekly mean (Tuesday-based week start to match USDM cadence)
2) USDM weekly cleanup
3) Merge by week and state for selected counties

Outputs:
- Integrated_weekly_KAN_20counties.csv
- Integrated_weekly_NEB_20counties.csv
"""
...
STATE_CONFIGS = [
    StateConfig(
        ...
        output_path="Integrated_weekly_KAN_20counties.csv",
        county_selection_path="County_selection_KS_20.csv",
    ),
    StateConfig(
        code="NE",
        nasa_path="NASA_POWER_daily_2010_2025_NEB.csv",
        usdm_path="NEB_dm_export_20100101_20251231.csv",
        output_path="Integrated_weekly_NEB_20counties.csv",
        county_selection_path="County_selection_NE_20.csv",
    ),
]
...
def main() -> None:
    ...
    print(f"County selection target: {TARGET_COUNTIES_PER_STATE} counties per state")

    selected_by_state: dict[str, pd.DataFrame] = {}

    for cfg in STATE_CONFIGS:
        usdm_weekly = prepare_usdm_weekly(base_dir / cfg.usdm_path, cfg.code)
        available = usdm_weekly["FIPS"].nunique()
        if available < TARGET_COUNTIES_PER_STATE:
            raise ValueError(
                f"[{cfg.code}] has only {available} unique counties, "
                f"cannot select {TARGET_COUNTIES_PER_STATE}."
            )

        selected = select_counties(usdm_weekly, TARGET_COUNTIES_PER_STATE)
