# Auto-generated from BiLSTM_Weekly_Nebraska_Clean.ipynb
# Scenario 3: Macro F1-driven feature selection with subset search
# Conversion: extracted code cells in original order with feature selection via permutation importance.
...
OUTPUT_FOLDER = os.path.join(NEBRASKA_DIR, 'output_weekly_nebraska_scenario3')
SCENARIO_NAME = 'Scenario 3 - Macro F1-driven Feature Selection'

# Scenario 3: Macro F1-driven feature selection
EPOCHS_BASELINE = 40  # Quick baseline for ranking only
EPOCHS_SUBSET_SEARCH = 40  # Quick training for subset evaluation
SUBSET_SIZES = [30, 25, 20, 15, 10]
PERMUTATION_REPEATS = 1  # Number of permutation repeats for stability
...
# %% [SCENARIO 3] Feature Selection Phase 1: Quick baseline training for feature ranking
print('\n' + '=' * 80)
print('SCENARIO 3: Feature Selection via Permutation Importance + Subset Search')
print('=' * 80)

print('\n--- Phase 1: Quick baseline training (for feature ranking) ---')
baseline_cfg = TRIAL_CONFIGS[1]  # Use the middle trial config for quick baseline
print(f'Using baseline config: {baseline_cfg["name"]}')

X_train_bal, y_train_bal = prepare_training_data(X_train_seq, y_train_seq, baseline_cfg['balancer'], SEED + 1)
y_train_enc = to_categorical(y_train_bal, num_classes=num_classes)

focal_alpha_baseline = compute_focal_alpha(
    y_train_bal,
    n_classes=num_classes,
    mode=baseline_cfg['focal_alpha_mode'],
    manual_alpha=baseline_cfg['manual_focal_alpha'],
)
...
baseline_model = build_model(
    ...
    lr=baseline_cfg['lr'],
    focal_gamma=baseline_cfg['focal_gamma'],
    focal_alpha=focal_alpha_baseline,
)

fit_kwargs = {}
if baseline_cfg['use_class_weight']:
    class_weight_dict = compute_class_weights(y_train_bal)
    fit_kwargs['class_weight'] = class_weight_dict

print(f'Training baseline model with {EPOCHS_BASELINE} epochs...')
baseline_history = baseline_model.fit(
    X_train_bal, y_train_enc,
    validation_data=(X_val_seq, y_val_enc),
    epochs=EPOCHS_BASELINE,
    batch_size=BATCH_SIZE,
    callbacks=[MacroF1Callback(X_val_seq, y_val_enc)],
    verbose=0,
    **fit_kwargs,
)
...
# %% [SCENARIO 3] Phase 2: Permutation importance
...
def compute_permutation_importance(model, X_val, y_val, feature_list, n_repeats=1, seed=SEED):
    ...
    for feat_idx, feat_name in enumerate(feature_list):
        ...
        for _ in range(n_repeats):
            ...
            rng = np.random.default_rng(seed + feat_idx + _)
            # Permute this feature across all timepoints and samples
        ...
        importances[feat_name] = np.mean(importance_scores)
    
    return importances

permutation_importance = compute_permutation_importance(
    baseline_model, X_val_seq, y_val_seq, feature_cols, n_repeats=PERMUTATION_REPEATS, seed=SEED
)
...
# %% [SCENARIO 3] Phase 3: Build feature subsets and evaluate
print('\n--- Phase 3: Evaluating feature subsets ---')

ranked_features = [feat for feat, _ in sorted_importance]
subset_results = []

for k in SUBSET_SIZES:
    k_actual = min(k, len(ranked_features))
    subset_features = ranked_features[:k_actual]
    
    print(f'\nEvaluating Top-{k_actual} subset ({len(subset_features)} features)...')
    ...
    # Train on subset with short epochs
    y_train_sub_enc = to_categorical(y_train_sub, num_classes=num_classes)
    y_val_sub_enc = to_categorical(y_val_sub, num_classes=num_classes)
    
    subset_model = build_model(
        SEQ_LENGTH,
        len(subset_features),
        num_classes,
        lstm_units=(64, 32),
    ...
    subset_model.fit(
        X_train_sub, y_train_sub_enc,
        validation_data=(X_val_sub, y_val_sub_enc),
        epochs=EPOCHS_SUBSET_SEARCH,
        batch_size=BATCH_SIZE,
        callbacks=[MacroF1Callback(X_val_sub, y_val_sub_enc)],
        verbose=0,
    )
    ...
    subset_results.append({
        ...
        'val_f1': subset_val_f1,
    })
    print(f'  → Validation Macro F1: {subset_val_f1:.4f}')

# %% [SCENARIO 3] Phase 4: Select best subset
print('\n--- Phase 4: Selecting best subset ---')

print(f'\nSubset evaluation results:')
for result in sorted(subset_results, key=lambda x: x['val_f1'], reverse=True):
    print(f'  Top-{result["size"]:2d}: Validation Macro F1 = {result["val_f1"]:.4f}')

best_subset_result = max(subset_results, key=lambda x: x['val_f1'])
feature_cols_best = best_subset_result['features']
best_subset_size = best_subset_result['size']
best_subset_f1 = best_subset_result['val_f1']

print(f'\nBest subset: Top-{best_subset_size} with validation macro-F1 = {best_subset_f1:.4f}')
print(f'Selected {len(feature_cols_best)} features:')
for i, feat in enumerate(feature_cols_best, 1):
    print(f'  {i:2d}. {feat}')

# Replace feature_cols for final training
feature_cols = feature_cols_best

# %% [SCENARIO 3] Phase 5: Re-prepare data with best subset and final training
...
# Run full trials on best subset
trial_results = []
best_trial = None
...
print('\nStarting hyperparameter trials on best subset...')
...
subset_result_lines = [
    f'Top-{result["size"]:02d}: Validation Macro F1 = {result["val_f1"]:.4f}'
    for result in sorted(subset_results, key=lambda item: item['val_f1'], reverse=True)
]
...
write_results_summary(
    ...
    extra_sections=[
        ('Feature Selection', [
            ...
            f'Baseline model (for ranking): {baseline_cfg["name"]}, val macro-F1: {baseline_val_f1:.4f}',
            'Feature ranking method: Permutation Importance (macro F1 on validation)',
            f'Subset sizes evaluated: {SUBSET_SIZES}',
            f'Best subset selected: Top-{best_subset_size} ({len(feature_cols_best)} features) with val macro-F1 = {best_subset_f1:.4f}',
        ]),
        ('Subset Evaluation Results', subset_result_lines),
    ],
)
